...
def _body_pairs(
    ...
    exclude_signature = set(m.exclude_signature.numpy())
    geom_con = m.geom_contype.numpy() | m.geom_conaffinity.numpy()
    filterparent = not (m.opt_disableflags & mjDSBL_FILTERPARENT)
...
def _geom_pairs(
    ...
    for b1, b2 in _body_pairs(m):
        ...
        for g1, g2 in itertools.product(g1_range, g2_range):
            ...
            mask |= geom_contype[g2] & geom_conaffinity[g1]
            if not mask:
                continue
            yield g1, g2, t1, t2


def _get_ngeom_pair(m: types.Model) -> int:
...
def _get_ngeom_pair_type_offset(m: types.Model) -> np.ndarray:
...
class CollisionInput:
    def __init__(
        ...
        max_contact_points = d.contact_pos.shape[0]
...
def _collision(input: CollisionInput, output: CollisionOutput):
    if input.ngeom == 0:
        return True
